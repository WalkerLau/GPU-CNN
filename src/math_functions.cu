...
__host__ void convolute(float* A, float* B, float* C,
	...
	for(int ifm_lump = 0; ifm_lump < src_chn/para_chn; ++ifm_lump){
		...
		rearrange_A<<<dimGrid,dimBlock_RA>>>(A, ifmaps, src_w, para_chn, ifm_lump, block_num);
...
__global__ void rearrange_A(float* A, data_t* ifmaps, 
	...
	int cidx = bz*blockDim.z + tz; //int hidx = by*blockDim.y + ty; int widx = bx*blockDim.x + tx; 

	ifmaps[cidx*src_h*src_w + ty*src_w + tx] = 
		A[(ifm_lump*para_chn + tz)*src_h*src_w + ty*src_w + tx];
	__syncthreads();	
...
__global__ void conv_grid(data_t* A, float* filters, float*C,
	...
	int tidx = tz*blockDim.y*blockDim.x + ty*blockDim.x + tx;
